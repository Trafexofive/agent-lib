...
import { CommandPaletteAction, IconName } from '../types'; // Assuming IconName is in types

// --- SVG Icon Components (copied from SidebarNav for consistency, or use a shared icon library) ---
...
const WrenchScrewdriverIcon = (props: React.SVGProps<SVGSVGElement>) => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M11.42 15.17L17.25 21A2.652 2.652 0 0021 17.25l-5.83-5.83M11.42 15.17l2.496-3.03c.528-1.036.94-2.196 1.026-3.417l.668-11.578A1.708 1.708 0 0017.15-.009L5.572 1.659c-1.221.089-2.381.502-3.417 1.026L.099 5.18c-.983.983-.983 2.57 0 3.553L3.74 12.37c1.036.528 2.196.94 3.417 1.026l3.031 2.496m-3.582-3.582l.133.133m4.244-4.243l2.331 2.331" /></svg>;
...
const CogIcon = (props: React.SVGProps<SVGSVGElement>) => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12a7.5 7.5 0 0015 0m-15 0a7.5 7.5 0 1115 0m-15 0H3m18 0h-1.5m-15.036-7.126L4.536 3.874m14.928 1.002l-1.06-1.06M3.874 19.464l1.06-1.06m14.028-12.428l1.06 1.06M12 9.75v1.5m0 3v1.5m0-4.5v.01M6.126 6.126L7.186 7.186m9.628 9.628l1.06 1.06M6.126 17.874l1.06-1.06m9.628-9.628l1.06-1.06" /></svg>;
...
const ArrowRightIcon = (props: React.SVGProps<SVGSVGElement>) => <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" {...props}><path strokeLinecap="round" strokeLinejoin="round" d="M13.5 4.5L21 12m0 0l-7.5 7.5M21 12H3" /></svg>;
...
const commandIconMap: Record<IconName, React.ElementType> = {
...
const CommandPalette: React.FC<CommandPaletteProps> = ({ show, onClose, actions, onExecuteAction }) => {
  ...
  const filteredActions = actions.filter(action => {
    ...
    return (
      ...
      (action.keywords && action.keywords.some(keyword => keyword.toLowerCase().includes(term))) ||
      (action.section && action.section.toLowerCase().includes(term))
    );
  }).sort((a,b) => {
    const sectionA = a.section || 'ZZZ'; // Put items without section last
  ...
  }, [onClose, filteredActions, selectedIndex, onExecuteAction]);

  if (!show) return null;

  const groupedActions: Record<string, CommandPaletteAction[]> = {};
  filteredActions.forEach(action => {
    const section = action.section || 'Other Actions';
  ...
  const actionSections = Object.keys(groupedActions).sort();
  let currentIndexCounter = -1;

  return (
    <div className="fixed inset-0 bg-slate-900/70 backdrop-blur-sm z-50 flex items-start justify-center pt-16 sm:pt-24 p-4" onClick={onClose} role="dialog" aria-modal="true" aria-labelledby="command-palette-title">
      <div className="bg-[var(--theme-modal-bg)] w-full max-w-xl rounded-xl shadow-2xl border border-[var(--theme-border-primary)] overflow-hidden" onClick={e => e.stopPropagation()}>
        <div className="flex items-center border-b border-[var(--theme-border-primary)] px-4">
          ...
          <input
            ...
            className="w-full h-12 bg-transparent text-[var(--theme-input-text)] placeholder-[var(--theme-text-secondary)] focus:outline-none"
            value={searchTerm}
        ...
        {filteredActions.length > 0 ? (
          <ul ref={resultsRef} className="max-h-[60vh] overflow-y-auto p-2 text-sm" role="listbox">
            {actionSections.map(sectionName => (
                <div key={sectionName}>
                    {sectionName !== 'Other Actions' && (
                         <li className="px-3 py-1.5 text-xs font-semibold text-[var(--theme-text-secondary)] sticky top-0 bg-[var(--theme-modal-bg)] z-10" role="presentation">
                            {sectionName}
                        </li>
                    )}
                    {groupedActions[sectionName].map(action => {
                        currentIndexCounter++;
                        const currentItemIndex = currentIndexCounter;
                        const IconComponent = action.iconName ? commandIconMap[action.iconName] : ArrowRightIcon;
                        return (
                            <li
                            key={action.id}
                            onClick={() => onExecuteAction(action)}
                            className={`flex items-center justify-between px-3 py-2.5 rounded-md cursor-pointer ${selectedIndex === currentItemIndex ? 'bg-[var(--theme-accent-primary)] text-white' : 'text-[var(--theme-text-primary)] hover:bg-[var(--theme-bg-accent)]'}`}
                            role="option"
                            aria-selected={selectedIndex === currentItemIndex}
                            id={`command-action-${action.id}`}
                            >
                            <div className="flex items-center">
                                <IconComponent className={`w-5 h-5 mr-3 ${selectedIndex === currentItemIndex ? 'text-white' : 'text-[var(--theme-text-secondary)]'}`} />
                                <span className="truncate">{action.label}</span>
                            </div>
                            {action.type && <span className={`text-xs px-1.5 py-0.5 rounded ${selectedIndex === currentItemIndex ? 'bg-white/20' : 'bg-[var(--theme-bg-accent)] text-[var(--theme-text-secondary)]'}`}>{action.type}</span>}
                            </li>
                        );
                    })}
                </div>
          ...
          <p className="p-6 text-center text-[var(--theme-text-secondary)] text-sm">No actions found.</p>
        )}
         <div className="px-4 py-2 border-t border-[var(--theme-border-primary)] text-xs text-[var(--theme-text-secondary)] flex justify-between">
            <span>Use <kbd className="font-mono p-0.5 bg-[var(--theme-bg-accent)] rounded shadow">↑</kbd> <kbd className="font-mono p-0.5 bg-[var(--theme-bg-accent)] rounded shadow">↓</kbd> to navigate, <kbd className="font-mono p-0.5 bg-[var(--theme-bg-accent)] rounded shadow">Enter</kbd> to select.</span>
            <span><kbd className="font-mono p-0.5 bg-[var(--theme-bg-accent)] rounded shadow">Esc</kbd> to close.</span>
        </div>
      </div>
    </div>
  );
