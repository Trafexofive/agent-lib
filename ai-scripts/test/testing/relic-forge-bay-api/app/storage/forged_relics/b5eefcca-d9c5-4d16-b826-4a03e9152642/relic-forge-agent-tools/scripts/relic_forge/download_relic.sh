...
# Perform download using curl directly to handle file output
# The call_api helper is not suitable for direct file downloads as it tries to parse body
...
if [ $CURL_EXIT_CODE -eq 0 ]; then
    if [ "$HTTP_STATUS" -ge 200 ] && [ "$HTTP_STATUS" -lt 300 ]; then
        if [ -s "$OUTPUT_PATH" ]; then
            # Basic check if it's gzip, API could return JSON error with 200 for some misconfigurations
            ...
            # Download seemed to succeed (HTTP 2xx) but output file is empty.
        ...
        # HTTP error from API (e.g., 404 Not Found, 500 Server Error)
        # The $OUTPUT_PATH file likely contains the error response from the API (e.g., JSON error detail)
        ERROR_RESPONSE_BODY=$(cat "$OUTPUT_PATH" | tr -d '\n\r' | sed 's/\"/\\\"/g')
        if echo "$ERROR_RESPONSE_BODY" | jq -e . > /dev/null 2>&1; then # If error body is JSON
             echo "$ERROR_RESPONSE_BODY" | jq '.' # Output it as is
        else # Error body is not JSON
    ...
    # curl command itself failed (e.g., network error, couldn't connect)
    ...
    # Clean up empty file if curl failed and left one
